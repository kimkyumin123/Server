...
const updateReviewResult = async (e, loggedInUser) => {
  try {
    ...
    if (existReview.userId !== loggedInUser.id) {
      // 해당 리뷰 수정권한 확인
      logger.error(
        `${__dirname}|CheckPermission::Review_UserId:${existReview.userId}&&loggedInUser:${loggedInUser.id}`,
      );
    ...
    if (e.place) {
      ...
      if (resultPlace === process.env.Transaction_ERROR || resultPlace === process.env.CreateFail_Place) {
        // 장소 트랜잭션실패
        return resultPlace;
      }
    }
...
const updateReviewFN = async (_, { review }, { logger, loggedInUser }) => {
  ...
  for (const i in review) {
    ...
    if (
      result === process.env.NotFound_Review ||
      result === process.env.Transaction_ERROR ||
      result === process.env.CheckPermission
    ) {
      // 에러
      break;
    }
